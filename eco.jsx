...
// FIX: The 'leaflet' import has been entirely removed to resolve the final module resolution error.
// We rely on the global 'L' object provided by the CDN in the execution environment.

import { Zap, Trophy, MapPin, User, CheckCircle, Camera, Leaf, Trash2, BookOpen, Download, TrendingUp } from 'lucide-react';

// ====================================================================
// CORE PROTOTYPE SETUP AND MOCK DATA
// ====================================================================

// Firebase/Authentication Mocking (as per instructions, we omit the full backend setup)
const __app_id = 'eco-platform-demo';
const __initial_auth_token = 'mock-auth-token'; 
// In a real app, this would be used with Firebase Auth, but here we just mock the user.
...
// ====================================================================
// 2. AI IMAGE VERIFICATION LOGIC (Using Gemini API for image analysis)
// ====================================================================

// Helper function to convert base64 to ArrayBuffer (Not used here, but kept for context)
...
// Global API Key (As required, empty string will be populated by the Canvas environment)
const apiKey = "";
const GEMINI_API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;

/**
 * Calls Gemini API to analyze an image and identify its content.
 * @param {string} base64Image - Base64 string of the image (with mime type prefix).
 * @param {string} prompt - The specific instruction for the model.
 */
const analyzeImageWithGemini = async (base64Image, prompt) => {
    // Strip the mime type prefix (e.g., "data:image/jpeg;base64,")
...
// A simple button component with styling
...
// ====================================================================
// 1. Gamified Learning (Quiz) Component
// ====================================================================
...
// ====================================================================
// 2. AI Image Verification Component (Refactored for native input)
// ====================================================================

// NEW COMPONENT: Information Card for Plants/Waste
...
// NEW COMPONENT: Detailed Environmental Suggestions Card
...
const ImageVerificationPage = ({ userState, setUserState, setPage }) => {
    // Refs for native input elements
    const fileInputRef = useRef(null);
    const cameraInputRef = useRef(null); 

    const [imageSrc, setImageSrc] = useState(null);
    const [isLoading, setIsLoading] = useState(false);
    // Modified result state to hold the detailed JSON object
    const [result, setResult] = useState(null);
    const [error, setError] = useState(null);

    // Handle file upload/camera capture (they use the same handler now)
    ...
    // Function to trigger the native file chooser for camera
    const triggerCamera = () => {
    ...
    // Core verification logic using Gemini
    const handleVerification = async () => {
        ...
        // NEW PROMPT: Requesting detailed, structured JSON output
        ...
        try {
            ...
            let data = {};
            try {
                // Clean up any markdown code fences before parsing
            ...
            // Update user state and store action
            if (pointsEarned > 0) {
                setUserState(prev => {
                    const newTotalPoints = prev.totalPoints + pointsEarned;
            ...
            setIsLoading(false);
            setImageSrc(null); // Clear image after submission
        }
    };
    ...
    return (
        <div className="p-8 bg-white rounded-xl shadow-2xl max-w-4xl mx-auto">
            ...
            <div className="flex flex-col items-center space-y-4 mb-6 p-6 border border-gray-100 rounded-xl">
                ...
                {/* Input Options (Modified) */}
                {!imageSrc && (
                    <div className="w-full space-y-4">
                        {/* Hidden input to capture video/camera stream */}
                        <input 
                            type="file" 
                            accept="image/*" 
                            capture="environment" // Hint to use the rear camera
                            ref={cameraInputRef} 
...
// ====================================================================
// 3. Mission Tracker with Map Component (REFACTORED for pure JS/CDN and GPS fallback)
// ====================================================================

const MissionTrackerPage = ({ userState, setUserState, setPage }) => {
    const mapRef = useRef(null); // Ref for the map container
    const mapInstanceRef = useRef(null); // Ref to hold the L.map instance
    ...
    // FIX 1: Use useMemo for calculation that depends on props/state to prevent infinite loop
    ...
    // FIX 2: Restrict the useEffect dependencies to only run when the mission list or current location changes
    useEffect(() => {
        // --- PURE JS LEAFLET INITIALIZATION ---
        // Check if L.map is available before attempting to initialize
        if (typeof L !== 'undefined' && typeof L.map === 'function' && mapRef.current) {
            
            // --- FIX for Leaflet marker assets ---
            ...
            try {
                ...
                // 2. Add Tile Layer (Map Tiles)
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                    attribution: '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
                }).addTo(map);

                // 3. Add Markers for Completed Missions
                ...
                // 4. Add Marker for Current GPS Location
                if (currentLocation) {
    ...
    // Dependency array relies on stable references (userState.missions, currentLocation)
    ...
    const getGpsLocation = (callback) => {
        if (!navigator.geolocation) {
            ...
            const mockLocation = { latitude: 28.6139, longitude: 77.2090 }; // Mock location (e.g., New Delhi)
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                setCurrentLocation(loc); // Update state to trigger map re-render
                setIsGpsLoading(false);
                callback(loc);
            },
            (error) => {
                // FIX: Log error object instead of just passing it to the console error function.
    ...
    const handleMissionCompletion = (missionId, reward) => {
        getGpsLocation((location) => {
            if (location) {
                ...
                // Update completed actions history for user feedback/modal display
...
const LeaderboardPage = ({ userState }) => {
    // Mock other users for a realistic leaderboard
    const mockUsers = [
...
const App = () => {
    ...
    const Dashboard = ({ userState, setPage }) => (
        <div className="p-4 md:p-8 bg-white rounded-xl shadow-2xl">
            {/* Landing Page Hero Section (matching new 11.jpg) */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
                ...
                <div className="py-12">
                    ...
                    <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                        ...
                        <button
                            onClick={() => setPage('Dashboard')} // Placeholder action
                ...
                {/* Right: Placeholder Image Container (Replaces new 11.jpg illustration) */}
            ...
            {/* How It Works / Feature Cards (Matching new 13.jpg) */}
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 pt-8 font-sans">
            ...
            <footer className="text-center text-gray-500 text-sm mt-8 pb-4">
                <p>Prototype developed using React, Tailwind CSS, and Gemini API for image analysis.</p>
            </footer>
        </div>
    );
};
